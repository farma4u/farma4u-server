...
async function getAllHinovaMembersService (clientHinovaToken: string): Promise<IHinovaMember[]> {
  ...
  try {
    ...
    // Impede a execução do loop while em casos em que não há associados na hinova para esse cliente
    if (membersPagesTotalCount === 0) isFirstIteration = false
  } catch (error) {
    logger.error(
...
export async function processHinovaMembersListService (): Promise<void> {
  ...
  try {
    ...
    logger.debug(hinovaClientsWithToken, 'Lista de clientes com integração com a Hinova')
    ...
    for (const hinovaClient of hinovaClientsWithToken) {
      ...
      try {
        ...
        logger.debug(hinovaMembers, `Lista de associados Hinova do cliente ${hinovaClient.fantasyName} - ${hinovaClient.cnpj}`)
...
// Rotina para atualizar associados da Hinova todos os dias, às 01:15 BRT (04:15 UTC)
export const hinovaMembersUpdateScheduleService = cron.schedule('15 4 * * *', async () => {
